<template>
<v-layout class="post-form">
    <v-dialog v-model="addPost" transition="dialog-bottom-transition">
        <i v-if="!addPost" slot="activator" @click="addPost = !addPost" class="btn btn-info fas fa-plus add-post-btn"></i>
        <i v-else slot="activator" @click="addPost = !addPost" id="minus-form" class="btn btn-danger fas fa-plus add-post-btn"></i>
        <v-card>
            <v-card-title id="form-title">create a post</v-card-title>
            <form id="form-body">
                <v-text-field v-model="post.title" maxlength="30" placeholder="title" type="string" required></v-text-field> 
                <v-textarea v-model="post.content" outline maxlength="140" counter placeholder="post content" clearable clear-icon="X" type="string" required></v-textarea> 
                <v-radio-group v-model="post.category" label="#category">
                    <v-radio append-icon="this" v-for="c in catOptions" :key="c" :label="c" :value="c" required></v-radio>
                </v-radio-group>
                <v-flex justify-content-center>
                    <v-btn round color="#18bc9c">submit</v-btn>
                </v-flex>
            </form>
        </v-card>
    </v-dialog>    
</v-layout>
</template>

<script>
export default {
    name: 'postForm',
    data(){
        return {
        addPost: false,
        post: {
            title: '',
            content: '',
            category: ''
        },
        catOptions: ["event", "lost and found", "traffic update", "neighborhood watch"]
        }
    }
}
</script>

<style>
#form-title {
    background-color: #3498db;
}
#form-body {
    background-color: #7cbce8;
}
#minus-form {
    transform: rotate(45deg)
}
</style>
